...
export default function PreviousImages() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header
        className={`sticky top-0 z-10 border-b bg-black/95 backdrop-blur-sm ${config?.appBackground} ${config?.borderColor}`}
      >
        <div
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h1 className={`text-lg font-semibold ${config?.headingColor}`}>
              Search History
            </h1>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex flex-col items-start gap-2 p-4">
                  <div className="flex w-full justify-between">
                    <div>
                      <h3 className="line-clamp-1 font-medium text-gray-400">
                        {item.caption}
                      </h3>
                    </div>
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() =>
                          router.push(`/imageGallery?imageId=${item.id}`)
                        }
                      >
      ...
      <footer
        className={`mb-16 border-t px-6 py-4 pb-[env(safe-area-inset-bottom)] ${config?.borderColor}`}
      >
        <div className="container flex items-center justify-between">
          ...
          <button
            className={`rounded-sm border px-3 py-1 ${config?.borderColor} text-red-400 hover:bg-red-900/20`}
            onClick={async () => {
              try {
